...
void main() {
  ...
  test(
    'lists workshops, products, targets, and component cards for Android selection flow',
    () async {
      ...
      final department =
          (await database.select(database.departments).get()).single;
      ...
      final product = (await objectsRepository.listActiveObjects()).firstWhere(
      ...
      final componentDir = Directory(
        paths.resolveRelativePath('media/components/${component.id}'),
      );
      await componentDir.create(recursive: true);
      final imageFile = File(
        '${componentDir.path}${Platform.pathSeparator}bearing.png',
      );
      ...
      final products = await inspectionsRepository.listProductsForWorkshop(
        workshop.id,
      );
      ...
      final targets = await inspectionsRepository
          .listSelectableInspectionTargets(product.id);
      expect(targets.map((item) => item.targetObjectId), contains(machine.id));

      final components = await inspectionsRepository.listComponentsForTarget(
        machine.id,
      );
  ...
  test(
    'starts draft, resolves checklist bindings, and resumes existing draft',
    () async {
  ...
  test(
    'saves answers and reflects progress in draft summaries and diagnostics',
    () async {
      final fixture = await _seedInspectionFixture(
        objectsRepository: objectsRepository,
        componentsRepository: componentsRepository,
      ...
      final textItem = draft.items.firstWhere(
        (item) => item.resultType == 'text',
      );
      ...
      final updatedDraft = await inspectionsRepository.loadInspection(
        draft.inspection.id,
      );
      ...
      expect(
        updatedDraft.items
            .firstWhere((item) => item.answerId == textItem.answerId)
            .comment,
        'Serial confirmed',
      );
  ...
  test(
    'saves signatures, generates pdf, queues completed inspection, and locks editing',
    () async {
      final fixture = await _seedInspectionFixture(
        objectsRepository: objectsRepository,
        componentsRepository: componentsRepository,
      ...
      final pdfInfo = await inspectionsRepository.generatePdf(
        draft.inspection.id,
      );
      ...
      expect(
        Directory(
          paths.resolveRelativePath(queue.single.localPath),
        ).existsSync(),
        isTrue,
      );
  ...
  test(
    'lists completed inspections for Windows history view with inspector details',
    () async {
      final fixture = await _seedInspectionFixture(
        objectsRepository: objectsRepository,
        componentsRepository: componentsRepository,
      ...
      await inspectionsRepository.saveSignature(
        inspectionId: draft.inspection.id,
        input: const InspectionSignatureInput(
          signerUserId: 'user-default-admin',
          signerName: 'Default Administrator',
      ...
      await inspectionsRepository.completeInspection(
        inspectionId: draft.inspection.id,
        actorUserId: 'user-default-admin',
      );

      final allResults = await inspectionsRepository.listResults();

      expect(allResults, hasLength(1));
      expect(allResults.single.inspectionId, draft.inspection.id);
      expect(allResults.single.userId, 'user-default-admin');
      expect(allResults.single.userName, 'Default Administrator');
      expect(allResults.single.hasPdf, isTrue);
      expect(allResults.single.signatureCount, 1);
    },
  );
}
